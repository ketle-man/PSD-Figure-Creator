...
- **绑定系统** — 直接在画布上为图层放置控制点：
  - **R**（蓝色）— 仅旋转
  - **MR**（红色/橙色）— 移动 ＋ 旋转
  - **SW**（绿色）— 切换：旋转手柄以步进切换最多 12 个槽位；可注册单个图层、自定义组 `[组]` 或 PSD 文件夹组 `[文件夹]`（组和文件夹按成员图层逐一展开为多个槽位）
...
```jsonc
{
  ...
  "sw_layers": [{
    ...
    "points": [{
      ...
      "radius": 60, "angle": 0,
      "groups": [
      "<layerId>",                               // 单个 PSD 图层 — 1 个槽位
      { "type": "custom_group", "id": "..." },   // 自定义组 — 每个成员图层 1 个槽位
      { "type": "psd_group",    "id": "..." }    // PSD 文件夹组 — 每个叶图层 1 个槽位
    ]
    }]
  }]
}
