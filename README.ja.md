...
Photoshop の「下のレイヤーにクリッピング」フラグが設定されたレイヤーは、レイヤーパネルと SW の `+L` ドロップダウンに **✂** バッジで表示されます。キャンバスコンポジターは `source-atop` ブレンドで描画します — 各クリッピングレイヤーは直下のベースレイヤーの不透明領域にマスクされます。クリッピングレイヤーに設置した R/MR リグはそのマスク領域内で通常通り機能します。
